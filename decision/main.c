...
static int receive_message(void){
  ...
  if(dst != MSTR) return 0; // not possible since every message is sent to the master and there is no communication between modules
...
// simple pseudo-random + send to motor (no response expected)
// simple pseudo-random + send to motor (no response expected)
...
successCounter++;                               // treat as success since we sent it
