...
static int receive_message(void)
{
  ...
  if (dst != MSTR)
    return 0; // not possible since every message is sent to the master or from the master
...
int main(void)
{
  ...
  {
    // text stuff
    draw_text(&g_disp, fx, x, y, "COMMUNICATION DEMO MODE[R]", RGB_GREEN);
    y += fh;
    draw_text(&g_disp, fx, x, y, "[BOOT]: requesting random", RGB_WHITE);
    y += fh;
    draw_text(&g_disp, fx, x, y, "data from all modules", RGB_WHITE);
    y += fh;
    y += fh;
    ...
    if (v >= 0)
    {
      ...
      strcat(buf, num);
      draw_text(&g_disp, fx, x, y, buf, RGB_GREEN);
    }
    else
    {
    ...
    if (v >= 0)
    {
      ...
      strcat(buf1, num1);
      draw_text(&g_disp, fx, x, y, buf1, RGB_GREEN);
    }
    else
    {
    ...
    if (successCounter == 1)
    {
      draw_text(&g_disp, fx, x, y, "DEMO FAILED", RGB_RED);
      y += fh;
    }
    if (successCounter == 3)
    {
      draw_text(&g_disp, fx, x, y, "DEMO PASSED", RGB_GREEN);
      y += fh;
    }
    while (get_switch_state(0) == 1)
      sleep_msec(10);
  }
  // second demo mode since i didnt want to delete the first one and design coach didnt like it
  else if (get_switch_state(1) == 1)
  {
    // text stuff
    draw_text(&g_disp, fx, x, y, "COMMUNICATION DEMO MODE", RGB_GREEN);
    y += fh;
    int y_live_hb1 = y + fh;
    int y_live_cry1 = y + 2 * fh;
    int y_live_mtr1 = y + 3 * fh;

    // Cradle Rocking Motion Frequency 0.2Hz 0.35Hz 0.5Hz 0.65Hz 0.70Hz
    // Cradle Rocking Motion Amplitude 20% 40% 60% 80% 100%
    // before the while(1), keep these persistent:
    uint8_t amp = 0;
    uint8_t freq = 0;
    ...
    while (1)
    {
      ...
      // edge-triggered button handling (no float into uint8_t)
      int b0 = get_button_state(0);
      int b1 = get_button_state(1);

      // An edge-triggered event happens only when a signal changes, not while it stays in the same state.
      // That “change” can be: Rising edge: when the signal goes from 0 → 1. Falling edge: when the signal goes from 1 → 0
      // So instead of reacting continuously while a button is held down, the program reacts once when it’s first pressed (or released).
      ...
      // --- redraw HUD ---
      clear_text_line(&g_disp, y_live_hb1, fh, RGB_BLACK);
      clear_text_line(&g_disp, y_live_cry1, fh, RGB_BLACK);
      clear_text_line(&g_disp, y_live_mtr1, fh, RGB_BLACK);
      ...
      itoa_u(last_bpm, num);
      strcat(buf, num);
      draw_text(&g_disp, fx, x, y_live_hb1, buf, RGB_WHITE);

      strcpy(buf, "[C] cry=");
      itoa_u(last_cry, num);
      strcat(buf, num);
      strcat(buf, "%");
      draw_text(&g_disp, fx, x, y_live_cry1, buf, RGB_WHITE);

      // motor line (no overlap, correct numbers)
      ...
      strcat(buf, num);
      strcat(buf, "%");
      draw_text(&g_disp, fx, x, y_live_mtr1, buf, RGB_WHITE);
    ...
    displayFillScreen(&g_disp, RGB_BLACK);
    y = fh * 1;
    draw_text(&g_disp, fx, x, y, "DECISION MAKING MODULE", RGB_GREEN);
    y += fh;
    draw_text(&g_disp, fx, x, y, "[BOOT]: pinging modules...", RGB_WHITE);
    y += fh;
    // pinging first to see if the ring works functional

    int y_hb = y;
    y += fh; // these are just spaces btf
    int y_cr = y;
    y += fh;
    int y_mt = y;
    y += fh;
    ...
    if (!hb_ok)
      draw_text(&g_disp, fx, x, y += fh, "[WARN] HB missing, BPM=80", RGB_YELLOW);
    if (!cry_ok)
      draw_text(&g_disp, fx, x, y += fh, "[WARN] CRY missing, %=0", RGB_YELLOW);
    if (!mtr_ok)
      draw_text(&g_disp, fx, x, y += fh, "[WARN] MOTOR missing", RGB_YELLOW);
